...
class DataFlowAnalysis {
    ...
    // The initial state of the analysis
		...
		void getIncomingEdges(unsigned index, std::vector<unsigned> * IncomingEdges) {
			...
			for (auto const &it : EdgeToInfo) {
				if (it.first.second == index)  // all the edges that has 'cur index' as the second edge value
  ...
  public:
    ...
    void runWorklistAlgorithm(Function * func) {
        ...
        for (auto const &it : EdgeToInfo) // all the edges in func
        {
            Edge edge=it.first;
            unsigned inputnode=edge.first; //index
            unsigned outputnode=edge.second; //index
            if (inputnode==0)
            {
                continue;
            }
        ...
        while (!worklist.empty())
        {
            ...
            unsigned firstInstrindex=worklist.front(); // cur node's index
            worklist.pop_front();
            vector<Info *> outInfo;
            Instruction * firstInstr= IndexToInstr[firstInstrindex]; //cur instruction in the node
            getIncomingEdges(firstInstrindex,&IncomingEdges); //cur node's incoming edges(node)
            ...
            for (unsigned i = 0; i < outInfo.size();i++)
            {
                ...
                Edge Outedge=std::make_pair(firstInstrindex,OutgoingEdges[i]); //cur node & the i th outgoingnode
                Info::join(outInfo[i],EdgeToInfo[Outedge],outputInfo);
                if (!Info::equals(EdgeToInfo[Outedge],outputInfo)) // outInfo[i] not empty (curnode->nextnode has new updated content)
                    {
                        EdgeToInfo[Outedge]=outputInfo;  // update the info of the edge<cur,next>
                        worklist.push_back(Outedge.second);   //put the next node into set fot recomputing
                    }
            }
        }
